...
\newglossaryentry{xml}{
  name=XML, 
  description={From eXtensible Markup Language, it is a language to encode the documents. It is composed by elements which starts and ends with tags. Example of element is \emph{<name>Joseph</name>}.}
}

\newglossaryentry{resource-based-model} {
  name=resource-based model, 
  description={Resource-based model is a design of an architecture which operates over the resources. Resource is a typed object containing data.}
}
...
\newglossaryentry{adapter}{ 
  name=adapter, 
  description={Adapter is a design pattern which serves to transform data. It allows two incompatible interfaces to communicate.}
}
...
\newglossaryentry{framework}{
  name=framework,
  description={A tool to ease the software development, in typically contains supporting libraries, tools and/or best practices for development.}
}
...
name=deprecation,
description={Deprecated means that the service or service method is still present in new version and can be still used by consumers to do not harm them. Consumers should not count with it but should implement new functionality in short future. Deprecated functionality will be removed at the point it is no longer used.}
}
...
name=method,
description={Relating the client-server communication over the network a method is an operation. It is part of the request sent from client to server and defines what operation soul be executed over the data on the server.}
}
...
description = {Design Pattern is a solution for commonly occurring problem in software engineering. Design patterns are reusable and helps avoiding to repeat solving of the same problem.}
}

\newglossaryentry{dal}{
name=DAL,
description={Data access layer usually stand between application layer and Data layer. It mediates communication between them. The DAL prevents the SQL injection (unauthorized access to database) and ensures the following of best practices related to data manipulation.}
}
...
name=domain name,
description={Domain name is the required part of the URL address. For exapmle \emph{www.mydomain.com}. It is possible to replace it by IP address which is a numerical representation of the address.}
}
